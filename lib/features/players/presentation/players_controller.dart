...
class PlayersController extends FamilyAsyncNotifier<List<Player>, String> {
  ...
  Future<Player> createPlayer(String name) async {
    final player = await _repo.createPlayer(_groupId, name);
    await refresh();
    return player;
  }
  ...
  /// Traspasa el historial de [mergeFromPlayerId] a [keepPlayerId], y borra
  /// al viejo — se usa cuando el capitán se da cuenta de que un jugador
  /// recién creado en realidad ya existía en el grupo bajo otro perfil.
  Future<void> mergePlayers({
    required String keepPlayerId,
    required String mergeFromPlayerId,
  }) async {
    await _repo.mergePlayers(
      _groupId,
      keepPlayerId: keepPlayerId,
      mergeFromPlayerId: mergeFromPlayerId,
    );
    await refresh();
  }
}
